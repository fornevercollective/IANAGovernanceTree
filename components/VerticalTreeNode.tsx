...
export function VerticalTreeNode({ id, name, description, children, level, onNodeSelect }: TreeNodeProps) {
  ...
  return (
    <div className="vertical-tree-node w-full">
      <div 
        className={cn(
      ...
      >
        <div style={{ marginLeft: `${level * 20}px` }} className="flex items-center gap-2 w-full">
          {hasChildren ? (
            <Button 
              variant="ghost" 
              size="icon" 
      ...
      {expanded && hasChildren && (
        <div className="pl-4 border-l border-border/50 ml-3">
          {children.map((child) => (
            <VerticalTreeNode 
              key={child.id} 
              {...child} 
